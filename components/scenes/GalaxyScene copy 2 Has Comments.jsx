...
const GalaxyScene = ({ data }) => {
  ...
  const [pauseAutoRotation, setPauseAutoRotation] = useState(true);
  ...
  const toNoTransition = (animate = true) => {
    tl.current = gsap.timeline({
      defaults: { duration: 1.2, ease: "expo.inOut" },
      // onStart: () => stopAllTimelineExcept(tl.current),
      onComplete: () => animate && transitionComplete(),
    });

    tl.current
      .set(starsRef.current.scale, { x: 5, y: 5, z: 5 })
      .set(dragWrapperRef.current.scale, { x: 0, y: 0, z: 0 })
      .set(sunRef.current.position, { x: 0, y: 0, z: 0 })
      ...
      .set(planetsOrbitRef.current.scale, { x: 0, y: -1, z: 0 });
    ...
    //   defaults: { duration: 1.2, ease: "expo.inOut" },
    //   onStart: () => stopAllTimelineExcept(noTransitionTL.current),
    //   onComplete: () => animate && transitionComplete(),
    // });

    // noTransitionTL.current
    //   .set(starsRef.current.scale, { x: 5, y: 5, z: 5 })
    //   .set(dragWrapperRef.current.scale, { x: 0, y: 0, z: 0 })
    //   .set(sunRef.current.position, { x: 0, y: 0, z: 0 })
    //   .set(moonRef.current.position, {
  ...
  const toHome = contextSafe(() => {
    ...
    toNoTransition(false);
    tl.current = gsap.timeline({
      defaults: { duration: 1.2, ease: "expo.inOut" },
      onStart: () => {
        // stopAllTimelineExcept(tl.current);
      },
    ...
    //   onStart: () => {
    //     toNoTransition(false);
    //     stopAllTimelineExcept(homeTL.current);
    //   },
    //   onComplete: () => transitionComplete(),
    // });
  ...
  const homeToPlanet = contextSafe(() => {
    planetsTL.current = gsap.timeline({
      ...
      onStart: () => {
        ...
        stopAllTimelineExcept(planetsTL.current);
      },
      onComplete: () => transitionComplete(),
    });
              ...
              <group ref={planetsOrbitRef} position-y={-1} scale={3}>
                {planets_data.map((planet, i) => (
                  <Planet
                    ...
                    scale={
                      pauseAutoRotation && i === activePlanet
                        ? 6
                        : w > h
                          ? 1
                          : 1.25
                    ...
                    largeDots={pauseAutoRotation && i === activePlanet}
                    hide={pauseAutoRotation && i !== activePlanet}
                  />
                ))}
              </group>
            </group>
